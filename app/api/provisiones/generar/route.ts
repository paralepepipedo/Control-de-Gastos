...
export async function POST(request: Request) {
  try {
    ...
    const { data: gastosFijos, error: errorFijos } = await supabase.from('gastos_fijos')
      .select('*')
      .eq('activo', true);
    ...
    if (yaExisten && modo === 'regenerar') {
      const { error: errorDeleteProv } = await supabase.from('provisiones')
        .delete()
        .eq('mes', mes)
        .eq('anio', anio);
    ...
    const { data: gastosExistentes, error: errorGastosExistentes } = await supabase.from('gastos')
